...
module vga_controller_640_60(
    ...
    always @(posedge pixel_clk) begin
        if (rst)
            ...
            hs <= (hcount >= HFP && hcount < HSP) ? 1'b0 : 1'b1; // Active low sync pulse
    end
    ...
    always @(posedge pixel_clk) begin: v_count
        if (rst)
            ...
            if (vcount == VMAX)
                ...
                vcount <= vcount + 1;  // Increment vcount at the end of each line
        end
    end
    ...
    always @(posedge pixel_clk) begin
        if (rst)
            vs <= 1'b1;
        else if (hcount == HMAX - 1)  // Update vs at the end of the line
